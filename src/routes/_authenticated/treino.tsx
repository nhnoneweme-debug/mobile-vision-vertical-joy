...
type Prog = Record<
  string,
  { done: boolean; series: number; weights: number[]; reps_done: number[] }
>;
...
function TreinoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setSessions(await fnSessions({ data: { planId: activeId } }));
      } catch {
        /* noop */
      }
    })();
  ...
  async function saveDays(days: WorkoutDay[]) {
    ...
    try {
      replacePlan(await fnUpdate({ data: { id: activePlan.id, days } }));
    } catch (e) {
  ...
  async function handleCreatePlan() {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Erro ao criar treino.");
    }
  }
  ...
  async function handleRename(name: string) {
    if (!activePlan) return;
    try {
      replacePlan(await fnUpdate({ data: { id: activePlan.id, name } }));
    } catch {
      /* noop */
    }
  }
  function addDay() {
    ...
    const days = [
      ...activePlan.days,
      { id: uid(), dia: `Dia ${activePlan.days.length + 1}`, foco: "", exercicios: [] },
    ];
    saveDays(days);
  ...
  function removeDay(i: number) {
    if (!activePlan) return;
    saveDays(activePlan.days.filter((_, idx) => idx !== i));
    setDayIdx(0);
  }
  ...
  function addExercise(i: number) {
    if (!activePlan) return;
    saveDays(
      activePlan.days.map((dd, idx) =>
        idx === i
          ? {
              ...dd,
              exercicios: [
                ...dd.exercicios,
                { id: uid(), nome: "Novo exercício", series: 3, reps: "10" },
              ],
            }
          : dd,
      ),
  ...
  function patchExercise(
    i: number,
    exId: string,
    patch: Partial<{ nome: string; series: number; reps: string }>,
  ) {
    if (!activePlan) return;
    saveDays(
      activePlan.days.map((dd, idx) =>
        idx === i
          ? {
              ...dd,
              exercicios: dd.exercicios.map((e) => (e.id === exId ? { ...e, ...patch } : e)),
            }
          : dd,
      ),
    );
  ...
  function openWeightModal(exId: string, exName: string, target: number, repsTarget?: string) {
    // Pré-preenche com a última série: repetir carga/reps é o caso comum.
    const last = prog[exId]?.weights?.slice(-1)[0];
    const lastReps = prog[exId]?.reps_done?.slice(-1)[0];
    setWeightInput(last != null ? String(last) : "");
    setRepsInput(lastReps ? String(lastReps) : "");
    setWeightModal({ exId, exName, target, repsTarget });
  }
  async function confirmSerie() {
    ...
    // Reps é opcional: em branco vai 0 e a série é registrada do mesmo jeito.
    const reps = Number(repsInput) || 0;
    setWeightModal(null);
    if (!active) startTimer();
    try {
      const r = await fnLogSerie({
        data: {
      ...
      setProg((p) => ({
        ...p,
        [exId]: { done: r.done, series: r.series, weights: r.weights, reps_done: r.reps_done },
      }));
    } catch {
      loadProgress();
    }
  ...
  async function removeSerie(exId: string) {
    ...
    try {
      const r = await fnUndoSerie({
        data: { planId: activePlan.id, dayKey: activeDay.id, exerciseKey: exId },
      });
      setProg((p) => ({
        ...p,
        [exId]: { done: r.done, series: r.series, weights: r.weights, reps_done: r.reps_done },
  ...
  async function confirmFinish() {
    ...
    setAccum(0);
    try {
      const s = await fnCreateSession({
        data: { planId: activePlan.id, durationSec, exercisesDone: done, exercisesTotal: total },
      });
      setSessions(await fnSessions({ data: { planId: activePlan.id } }));
      toast.success("Sessão registrada.");
      setCheckinSession((s as { id?: string })?.id ?? null);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Erro ao registrar sessão.");
    }
  }
    ...
    <MobileShell>
      ...
      {plans === null ? (
        ...
        <>
            ...
            <>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {activePlan.days.map((d, i) => (
                      <button
                        ...
                        onClick={() => setDayIdx(i)}
                        className={
                          "shrink-0 rounded-lg border px-3 py-1.5 font-display text-[11px] tracking-wide " +
                          (i === dayIdx
                            ? "border-ember/60 bg-ember/10 text-ember"
                            : "border-border text-muted-foreground")
                        }
                    ...
                    {editing && (
                      <button
                        type="button"
                        onClick={addDay}
                        className="shrink-0 rounded-lg border border-dashed border-ember/40 px-3 py-1.5 text-ember"
                      >
              ...
              {editing ? (
                <section className="space-y-4 px-4 pt-4">
                  <div>
                    <label className="font-display text-[10px] tracking-[0.3em] text-muted-foreground">
                      NOME DO PLANO
                    </label>
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (v && v !== activePlan.name) handleRename(v);
                      }}
                  ...
                  {activeDay ? (
                    <div className="forge-card rounded-2xl p-3">
                      <div className="mb-2 flex items-center gap-2">
                        <input
                          defaultValue={activeDay.dia}
                          onBlur={(e) =>
                            patchDay(dayIdx, { dia: e.target.value.trim() || activeDay.dia })
                          }
                          className="w-24 rounded-lg border border-border bg-charcoal-900 px-2 py-1.5 text-sm text-foreground focus:border-ember/60 focus:outline-none"
                        />
                        <input
                          defaultValue={activeDay.foco ?? ""}
                          placeholder="foco"
                          onBlur={(e) => patchDay(dayIdx, { foco: e.target.value })}
                          className="flex-1 rounded-lg border border-border bg-charcoal-900 px-2 py-1.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-ember/60 focus:outline-none"
                        />
                        <button
                          type="button"
                      ...
                      <div className="space-y-2">
                        {activeDay.exercicios.map((ex) => (
                          <div key={ex.id} className="flex items-center gap-2">
                            <input
                              ...
                              onBlur={(e) =>
                                patchExercise(dayIdx, ex.id, {
                                  nome: e.target.value.trim() || ex.nome,
                                })
                              }
                              className="min-w-0 flex-1 rounded-lg border border-border bg-charcoal-900 px-2 py-1.5 text-sm text-foreground focus:border-ember/60 focus:outline-none"
                            />
                            <input
                              type="number"
                              defaultValue={ex.series ?? 3}
                              onBlur={(e) =>
                                patchExercise(dayIdx, ex.id, {
                                  series: Number(e.target.value) || 0,
                                })
                              }
                              className="w-12 rounded-lg border border-border bg-charcoal-900 px-1.5 py-1.5 text-center text-sm text-foreground focus:border-ember/60 focus:outline-none"
                              aria-label="séries"
                            />
                            <input
                              defaultValue={ex.reps ?? ""}
                              placeholder="reps"
                              onBlur={(e) => patchExercise(dayIdx, ex.id, { reps: e.target.value })}
                              className="w-16 rounded-lg border border-border bg-charcoal-900 px-1.5 py-1.5 text-center text-sm text-foreground placeholder:text-muted-foreground focus:border-ember/60 focus:outline-none"
                        ...
                        <button
                          ...
                          className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-ember/40 py-2 font-display text-[11px] tracking-[0.2em] text-ember"
                        >
                    ...
                    <button
                      type="button"
                      onClick={addDay}
                      className="flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-ember/40 py-4 font-display text-sm tracking-wide text-ember"
                    >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[11px] tracking-[0.2em] text-muted-foreground active:text-destructive"
                  >
                ...
                <>
                  {activeDay && activeDay.exercicios.length > 0 ? (
                        ...
                        <div className="flex gap-2">
                          {!active ? (
                            <button
                              type="button"
                              onClick={startTimer}
                              className="flex items-center gap-2 rounded-xl bg-ember px-4 py-2.5 font-display text-sm tracking-wide text-charcoal-900 active:scale-[0.98]"
                            >
                              <Play className="h-4 w-4" strokeWidth={2.6} /> Iniciar
                            </button>
                          ) : (
                            <>
                              <button
                                ...
                                className="grid h-11 w-11 place-items-center rounded-xl border border-border bg-charcoal-900 text-foreground active:scale-95"
                              ...
                              <button
                                ...
                                className="flex items-center gap-2 rounded-xl border border-ember/50 bg-charcoal-900 px-4 py-2.5 font-display text-sm tracking-wide text-ember active:scale-[0.98]"
                              >
                                <Square className="h-4 w-4" strokeWidth={2.6} /> Finalizar
                              </button>
                            </>
                          )}
                        </div>
                        ...
                        {activeDay.exercicios.map((ex) => {
                          ...
                          return (
                            <div
                              key={ex.id}
                              className={
                                "forge-card flex items-center gap-3 rounded-2xl px-3.5 py-3 " +
                                (isFocus ? "ring-1 ring-ember/50" : "")
                              }
                            >
                              <span
                                className={
                              ...
                              <div className="min-w-0 flex-1">
                                <p
                                  className={
                                    "truncate font-display text-base tracking-wide " +
                                    (st.done
                                ...
                                <p className="text-[11px] text-muted-foreground">
                                  {st.series}/{target} séries{ex.reps ? ` · meta ${ex.reps}` : ""}
                                  {st.weights.length > 0
                                    ? ` · última ${st.weights[st.weights.length - 1]}kg${
                                        st.reps_done[st.reps_done.length - 1]
                                          ? `×${st.reps_done[st.reps_done.length - 1]}`
                                          : ""
                                      }`
                                    : ""}
                              ...
                              <div className="flex shrink-0 items-center gap-1.5">
                                {st.series > 0 && (
                                  <button
                                    type="button"
                                    onClick={() => removeSerie(ex.id)}
                                    className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-charcoal-900 text-muted-foreground active:scale-95"
                                ...
                                <button
                                  type="button"
                                  onClick={() => openWeightModal(ex.id, ex.nome, target, ex.reps)}
                                  disabled={st.done}
                                  className="grid h-9 w-9 place-items-center rounded-xl border border-ember/50 bg-charcoal-900 text-ember disabled:opacity-40 active:scale-95"
                    ...
                    <section className="px-4 pt-6">
                      <div className="forge-card rounded-2xl px-4 py-8 text-center text-muted-foreground">
                        {activePlan.days.length === 0
                          ? "Esse treino ainda não tem dias. Toque no lápis pra montar."
                          : "Esse dia não tem exercícios. Toque no lápis pra adicionar."}
                      </div>
                    </section>
                  )}
          ...
          <Link
            to="/assistente"
            className="forge-card mx-4 mb-8 mt-6 flex items-center gap-3 rounded-2xl p-3.5 active:forge-press-active"
          >
      ...
      {weightModal && (
        ...
        >
          <div
            className="w-full max-w-xs rounded-2xl border border-border bg-charcoal-900 p-4"
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground">
              Série {(prog[weightModal.exId]?.series ?? 0) + 1} de {weightModal.target}
              {weightModal.repsTarget ? ` · meta ${weightModal.repsTarget} reps` : ""}
            </p>
            <div className="mt-3 grid grid-cols-2 gap-2">
              <label className="block">
                <span className="mb-1 block font-display text-[9px] tracking-[0.25em] text-muted-foreground">
                  PESO (KG)
                </span>
                <input
                  autoFocus
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") confirmSerie();
                  }}
                  aria-label="Peso em quilos"
                  placeholder="0"
                  className="w-full rounded-xl border border-border bg-charcoal-800 px-3 py-2.5 text-center text-lg text-foreground focus:border-ember/60 focus:outline-none"
                />
              </label>
              <label className="block">
                <span className="mb-1 block font-display text-[9px] tracking-[0.25em] text-muted-foreground">
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="flex-1 rounded-xl bg-ember py-2.5 font-display text-sm tracking-wide text-charcoal-900 active:scale-[0.99]"
              >
                Registrar
              </button>
              <button
                type="button"
                onClick={() => setWeightModal(null)}
                className="rounded-xl border border-border bg-charcoal-800 px-4 py-2.5 font-display text-sm text-muted-foreground"
              >
                Cancelar
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            <div className="mt-3 space-y-1 text-sm text-foreground">
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">Séries feitas</span>
                <span>
                  {activeDay.exercicios.reduce((s, e) => s + (prog[e.id]?.series ?? 0), 0)}
                </span>
              </div>
            </div>
            <div className="mt-4 flex gap-2">
              <button
                ...
                className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-ember py-2.5 font-display text-sm tracking-wide text-charcoal-900"
              >
                <Check className="h-4 w-4" strokeWidth={3} /> Finalizar
              </button>
              <button
                type="button"
                onClick={() => setFinishOpen(false)}
