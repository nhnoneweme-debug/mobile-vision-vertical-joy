...
como uma fala que será lida em voz alta. Seja específica com o que foi dito.`;
...
export const liveHandoverReply = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabase.from("ai_audit_log").insert({
      ...
      status: "applied",
      payload: { turn: data.turn.slice(0, 2000), output: message, addressed } as never,
    });
    return { message: addressed ? message : "", addressed };
  });
...
export const liveSessionChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabase.from("ai_audit_log").insert({
      ...
      status: "applied",
      payload: { question: data.question, output: message } as never,
    });
    return { message };
  });
